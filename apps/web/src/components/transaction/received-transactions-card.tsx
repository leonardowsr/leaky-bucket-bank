...
interface ReceivedTransactionsCardProps {
	accountId: string;
}

export function ReceivedTransactionsCard({
	accountId,
	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>
					{transactions?.length
						? `${transactions.length} transações recebidas`
			...
			<ScrollArea className="h-96 w-full">
				{isLoading ? (
					...
					<div className="flex h-full items-center justify-center p-4">
						<p className="text-muted-foreground">Nenhuma transação recebida</p>
					</div>
				)}
			</ScrollArea>
		</Card>
